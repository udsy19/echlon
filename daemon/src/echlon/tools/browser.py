...
Session model: a single long-lived browser is held in a module global so refs
and page state persist across tool calls. By default we launch the user's
installed Chrome with a dedicated persistent profile (so logins survive across
runs and the user can watch). Set ECHLON_CHROME_CDP to attach to an
already-running Chrome instead.
...
class _Session:
    """Lazily-started, long-lived browser session shared across tool calls."""
...
def browser_snapshot() -> str:
    ...
    return _snapshot(_session.page())
